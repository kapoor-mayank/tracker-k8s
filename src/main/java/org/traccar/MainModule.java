...
public class MainModule
    ...
    public static Geocoder provideGeocoder(Config config) {
        if (config.getBoolean(Keys.GEOCODER_ENABLE)) {
            ...
            switch (type) {
                case "nominatim":
                    return (Geocoder) new NominatimGeocoder(url, key, language, cacheSize, addressFormat);
                case "gisgraphy":
                    return (Geocoder) new GisgraphyGeocoder(url, cacheSize, addressFormat);
                case "mapquest":
                    return (Geocoder) new MapQuestGeocoder(url, key, cacheSize, addressFormat);
                case "opencage":
                    return (Geocoder) new OpenCageGeocoder(url, key, cacheSize, addressFormat);
                case "bingmaps":
                    return (Geocoder) new BingMapsGeocoder(url, key, cacheSize, addressFormat);
                case "factual":
                    return (Geocoder) new FactualGeocoder(url, key, cacheSize, addressFormat);
                case "geocodefarm":
                    return (Geocoder) new GeocodeFarmGeocoder(key, language, cacheSize, addressFormat);
                case "geocodexyz":
                    return (Geocoder) new GeocodeXyzGeocoder(key, cacheSize, addressFormat);
                case "ban":
                    return (Geocoder) new BanGeocoder(cacheSize, addressFormat);
                case "here":
                    return (Geocoder) new HereGeocoder(id, key, language, cacheSize, addressFormat);
                case "mapmyindia":
                    return (Geocoder) new MapmyIndiaGeocoder(url, key, cacheSize, addressFormat);
            }
            return (Geocoder) new GoogleGeocoder(key, language, cacheSize, addressFormat);
        }
    ...
    public static GeolocationProvider provideGeolocationProvider(Config config) {
        if (config.getBoolean(Keys.GEOLOCATION_ENABLE)) {
            ...
            switch (type) {
                case "google":
                    return (GeolocationProvider) new GoogleGeolocationProvider(key);
                case "opencellid":
                    return (GeolocationProvider) new OpenCellIdGeolocationProvider(key);
                case "unwired":
                    return (GeolocationProvider) new UnwiredGeolocationProvider(url, key);
            }
            return (GeolocationProvider) new MozillaGeolocationProvider(key);
        }
